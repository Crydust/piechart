...
export class Rect {
	...
	subRect(top, right, bottom, left) {
		const subTop = this.getTop() + ((1 - top) * this.getHeight());
		const subHeight = this.getHeight() * (top - bottom);
		const subLeft = this.getLeft() + (left * this.getWidth());
		const subWidth = (right - left) * this.getWidth();
...
export function toRad(degrees) {
	let result = (degrees * Math.PI / 180) % (Math.PI * 2);
	if (result < 0) {
		result += (Math.PI * 2);
	}
	return result;
...
export function toDeg(radians) {
	let result = (radians * 180 / Math.PI) % 360;
	if (result < 0) {
		result += 360;
	}
	return result;
